...
const analyzeThemes = () => {
  ...
  if (lightThemeLightness.length > 0) {
    categorization.statistics.averageLightness.light = 
      Math.round(lightThemeLightness.reduce((a, b) => a + b, 0) / lightThemeLightness.length);
  }
  if (darkThemeLightness.length > 0) {
    categorization.statistics.averageLightness.dark = 
      Math.round(darkThemeLightness.reduce((a, b) => a + b, 0) / darkThemeLightness.length);
  }
...
const main = () => {
  ...
  console.log(`Light themes: ${categorization.statistics.lightThemes} (avg lightness: ${categorization.statistics.averageLightness.light}%)`);
  console.log(`Dark themes: ${categorization.statistics.darkThemes} (avg lightness: ${categorization.statistics.averageLightness.dark}%)`);
  ...
  // Print sample themes
  console.log('\n🌞 Sample Light Themes:');
  Object.entries(categorization.light).slice(0, 5).forEach(([name, info]) => {
    console.log(`  - ${name}: ${info.background} (L: ${info.lightness}%)`);
  });
  
  console.log('\n🌙 Sample Dark Themes:');
  Object.entries(categorization.dark).slice(0, 5).forEach(([name, info]) => {
    console.log(`  - ${name}: ${info.background} (L: ${info.lightness}%)`);
  });

...
const extractPrimaryColor = (themeData: any): string => {
  ...
  if (themeData.rules && Array.isArray(themeData.rules)) {
    for (const source of primarySources) {
      const rule = themeData.rules.find(
        (r: any) => r.token === source || (r.scope && r.scope.includes(source))
      );
...
const extractSemanticColor = (themeData: any, tokenType: string, fallback: string): string => {
  if (themeData.rules && Array.isArray(themeData.rules)) {
    const rule = themeData.rules.find(
      (r: any) => r.token === tokenType || (r.scope && r.scope.includes(tokenType))
    );
    if (rule && rule.foreground) {
      return rule.foreground.startsWith('#') ? rule.foreground : '#' + rule.foreground;
    }
  ...
  try {
      ...
      if (themeService) {
        ...
        if (themeService._theme) {
          ...
          if (currentTheme.themeData) {
            ...
            themeData = {
              base:
                currentTheme.base || (isBuiltIn && themeName.includes('light') ? 'vs' : 'vs-dark'),
        ...
        if (!themeData && themeService._themeRegistry) {
          ...
          const registeredTheme =
            registry.getTheme?.(themeName) || registry._themes?.get(themeName);
          if (registeredTheme) {
    ...
    if (!themeData.colors && themeData.rules) {
      ...
      if (!fgRule || foreground === '#f8f8f2') {
        const textRule = themeData.rules.find(
          (r: any) => r.token === 'source' || r.token === 'text' || r.token === 'variable'
        );
    ...
    const colors: ExtractedThemeColors = {
      ...
      sidebarForeground: ensureHash(themeData.colors?.['sideBar.foreground'], foreground),
      ...
      activityBarForeground: ensureHash(themeData.colors?.['activityBar.foreground'], foreground),
      ...
      editorGroupBackground: ensureHash(
        themeData.colors?.['editorGroup.background'] ||
          themeData.colors?.['editorGroup.header.tabsBackground'],
        background
      ),

      // Tab colors
      tabActiveBackground: ensureHash(themeData.colors?.['tab.activeBackground'], background),
      ...
      inputForeground: ensureHash(themeData.colors?.['input.foreground'], foreground),
      inputBorder: ensureHash(themeData.colors?.['input.border'], neutralScale[2]),
      ...
      contrastBorder: ensureHash(themeData.colors?.['contrastBorder'], neutralScale[3]),

      // Accent colors
      primary: extractPrimaryColor(themeData),
      selection: ensureHash(themeData.colors?.['editor.selectionBackground'], '#264f78'),
      ...
      badgeForeground: ensureHash(themeData.colors?.['badge.foreground'], '#ffffff'),
      buttonBackground: ensureHash(
        themeData.colors?.['button.background'],
        extractPrimaryColor(themeData)
      ),
      buttonForeground: ensureHash(themeData.colors?.['button.foreground'], '#ffffff'),
    };
...
export const applyThemeColors = (colors: ExtractedThemeColors) => {
  ...
  lightness.forEach((l, i) => {
    ...
    root.style.setProperty(`--primary-${shade}`, `${primaryHSL.h} ${saturation[i]}% ${l}%`);
  });
